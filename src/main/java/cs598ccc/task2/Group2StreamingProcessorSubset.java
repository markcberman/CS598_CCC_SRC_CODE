...
public class Group2StreamingProcessorSubset {

    private static Logger logger = Logger.getLogger(Group2StreamingProcessorSubset.class);
    ...
    private Integer enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions = null;
    private Integer enrichedOntimePerfTask2Group2Q4TopicMinPartitions = null;
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            Group2StreamingProcessorSubset instance = new Group2StreamingProcessorSubset();
    ...
    private void loadProperties(String propertyFileName) throws IOException, NumberFormatException {
        ...
        enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions = Integer.valueOf(prop.getProperty("enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions","12"));
        logger.info("enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions: " + enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions);
        enrichedOntimePerfTask2Group2Q4TopicMinPartitions = Integer.valueOf(prop.getProperty("enrichedOntimePerfTask2Group2Q4TopicMinPartitions", "12"));
        logger.info("enrichedOntimePerfTask2Group2Q4TopicMinPartitions: " + enrichedOntimePerfTask2Group2Q4TopicMinPartitions);
    ...
    private void start() throws StreamingQueryException {
        ...
        Dataset<Row> kafka_input = spark
                ...
                .option("subscribe", enrichedOntimePerfTask2Group2Q1Q2Topic.trim())
                .option("startingOffsets", startingOffsets)
                .option("minPartitions", enrichedOntimePerfTask2Group2Q1Q2TopicMinPartitions)
        ...
        Dataset<Row> kafka_input_q4 = spark
                .readStream()
                .format("kafka")
                .option("kafka.bootstrap.servers", kafkaHost)
                .option("subscribe", enrichedOntimePerfTask2Group2Q4Topic.trim())
                .option("startingOffsets", startingOffsets)
                .option("minPartitions", enrichedOntimePerfTask2Group2Q4TopicMinPartitions)
                .option("maxOffsetsPerTrigger", enrichedDataKafkaTopicMinmaxOffsetsPerTrigger)
                .load();

        Dataset<Row> enriched_ontime_perf__q4_df = kafka_input_q4.selectExpr("CAST(value AS STRING)", "CAST(timestamp AS TIMESTAMP)")
                .select(from_json(col("value"), SchemaCreator.createSchemaWithId()).as("data"), col("timestamp"))
                .select("data.*", "timestamp");

        enriched_ontime_perf__q4_df.printSchema();

        enriched_ontime_perf__q4_df.dropDuplicates("id");

        Dataset<Row> query2_4_unfiltered_results_df = enriched_ontime_perf__q4_df
                .groupBy(enriched_ontime_perf__q4_df.col("Origin"),enriched_ontime_perf__q4_df.col("Dest"))
                .agg(
                        avg(enriched_ontime_perf__q4_df.col("ArrDelay")).alias("avgArrivalDelay")
